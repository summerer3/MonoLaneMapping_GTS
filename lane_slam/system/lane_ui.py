...
class LaneUI:
    ...
    def visualize_map(self, show_id=False):
        ...
        for lane_id, lane_feature in self.lanes_in_map.items():
            lane_pts = lane_feature.points
            # print("lane_pts\n", lane_pts)
            ctrl_pts = lane_feature.get_ctrl_xyz()
            category = lane_feature.category
            # clr = [0.4, 0.4, 0.4]
            clr = [0, 1, 0] if category == 'lane_dashed' else [1, 0, 0]

            pcd = make_open3d_point_cloud(lane_pts, color=clr)
            ...
            # window_size = fitted_pts.shape[0] - 1 if fitted_pts.shape[0] // 2 == 0 else fitted_pts.shape[0]
            # fitted_pts[:, 0] = scipy.signal.savgol_filter(fitted_pts[:, 0], window_size, 5) 
            # fitted_pts[:, 1] = scipy.signal.savgol_filter(fitted_pts[:, 1], window_size, 5) 
            # print("fitted_pts:",fitted_ps)
            vis_points.append(fitted_pts)
            vis_category.append(clr)
        ...
        visualize_points_list(vis_points, vis_category, extra_pcd=vis_pcds, title = self.segment)
    ...
    def load_data(self):
        ...
        odom_path = os.path.join(self.bag_file, 'pose_new.txt')
        lane_pre_path = np.sort(glob.glob(os.path.join(self.bag_file, 'lidar/*.json')))
        
        odom = open(odom_path, 'r').readlines()
        idx = 0
        # idx = 1600
        # odom = odom[idx:idx+500]
        
        for idx_pose, pose in enumerate(odom):
            lanes_predict_msg = json.load(open(lane_pre_path[idx+idx_pose], 'r'))['3d_polyline']['annotations']
            timestamp = np.asarray(pose.strip().split(' ')[0], dtype='float')
            pose = np.asarray(pose.strip().split(' ')[1:], dtype='float')
            gt_pose_wc = posemsg_to_np(pose)
    ...
    def save_map(self):
        ...
        mkdir_if_missing(output_dir)
        # 改为输出为json文件
        json_name = os.path.join(output_dir, '{}.json'.format("map"))
        ...
        os.environ['GLOB_LANE_PATH'] = json_name

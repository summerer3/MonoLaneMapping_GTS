...
class LaneFeature:
    ...
    def smooth_ctrl_pts(self, ctrl_pts):
        '''
        对控制点进行曲线拟合，将异常点拉回原始轨迹中，实现平滑处理
        '''
        def refine_ctrl_pts(pts):
            idx_long = np.argmax(abs(pts[0, :2]-pts[-1, :2]))
            coefficients = np.polyfit(pts[:, idx_long], pts[:, 1-idx_long], 3)
            polynomial = np.poly1d(coefficients)
            pts[:, 1-idx_long] = polynomial(pts[:, idx_long])

            return pts
        
        num_ctrl = len(ctrl_pts)
        # clip_length 设置拟合曲线的点数，控制点的间距为3m
        clip_length = 6
        num_clip = (num_ctrl // clip_length) + 1
        if num_ctrl % clip_length == 0:
            num_clip -= 1
        for i_clip in range(num_clip):
            refine_pts = ctrl_pts[i_clip * clip_length:min((i_clip+1) * clip_length, num_ctrl)]
            refine_pts = refine_ctrl_pts(refine_pts)
            ctrl_pts[i_clip * clip_length + 1: min((i_clip+1) * clip_length, num_ctrl) - 1] = refine_pts[1:-1]
        return ctrl_pts
        
    
    def smooth(self, last=False):
        ctrl_pts = self.get_ctrl_xyz()
        if last:
            ctrl_pts = self.smooth_ctrl_pts(np.array(ctrl_pts))
            st_pnt = ctrl_pts[0] + 0.5 * (ctrl_pts[0] - ctrl_pts[min(6, len(ctrl_pts)-1)]) \
                / np.linalg.norm(ctrl_pts[0] - ctrl_pts[min(6, len(ctrl_pts)-1)])
            ed_pnt = ctrl_pts[-1] + 0.5 * (ctrl_pts[-1] - ctrl_pts[-min(6, len(ctrl_pts)-1)]) \
                / np.linalg.norm(ctrl_pts[-1] - ctrl_pts[-min(6, len(ctrl_pts)-1)])
            pop_idx = []
            dwn_num = 1
            if len(ctrl_pts) > 2:
                for i_ctrl in range(1, len(ctrl_pts)-1):
                    if not i_ctrl % dwn_num  == 0:
                        pop_idx.append(i_ctrl)
            ii = 0
            for idx in pop_idx:
                ctrl_pts.pop(idx - ii)
                ii += 1
            
            # ctrl_pts.insert(0, st_pnt)
            # ctrl_pts.insert(-1, ed_pnt)
    ...
    def add_points(self, new_points):
        if new_points.size > 0 :
            if np.linalg.norm(self.points[-1] - new_points[0]) \
                <= np.linalg.norm(self.points[0] - new_points[-1]):
                self.points = np.vstack([self.points, new_points])
            else:
                self.points = np.vstack([new_points, self.points])
            self.kdtree = KDTree(self.points)  # 更新 KD 树

    def get_non_overlap_points(self, other):
        non_overlap_points = []
        for point in self.points:
            if not self.is_point_in_overlap_region(point, other):
                non_overlap_points.append(point)
        return np.array(non_overlap_points)
    
    def get_overlap_points(self, other):
        overlap_points = []
        for point in self.points:
            if  self.is_point_in_overlap_region(point, other):
                overlap_points.append(point)
        return np.array(overlap_points)
    
    def is_point_in_overlap_region(self, point, other):
        if other.kdtree is None:
            return False
        dist_thd = cfg.lane_asso.lane_width / 2
        indices = other.kdtree.query_ball_point(point, r=dist_thd)
